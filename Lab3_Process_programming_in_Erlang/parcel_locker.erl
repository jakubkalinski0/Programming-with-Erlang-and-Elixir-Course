...
-export([get_random_coordinates/2, report/0, report/2,
    find_min_distance_sequential/2, find_min_distance_parallel/2,
    find_min_distance_n_threads/3]).
...
list_into_n_batches(List, N) ->
    ...
    Remaining =
        [[El] || El <- lists:sublist(List, N * Batch_size + 1, Length)] ++
...
find_min_distance_parallel(People_locations, Parcels_locations) ->
    ...
    Results = [receive Result -> Result end || _ <- People_locations],
    lists:min(Results).
...
find_min_distance_n_threads(People_locations, Parcels_locations, Number_of_threads) ->
    ...
    PID = self(),
    _Children =
        [spawn(fun() -> PID ! find_min_distance_parallel(People_batch, Parcels_locations) end)
            || People_batch <- Batches],
    Results = [receive Result -> Result end || _ <- lists:seq(1, Number_of_threads)],
    lists:min(Results).

report() ->
    report(20 * 1000, 1 * 1000).
